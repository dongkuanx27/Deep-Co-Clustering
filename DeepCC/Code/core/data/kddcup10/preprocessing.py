...
def preprocess():
    filename = '../../../../dataset/kddcup99-10/kddcup10.data_10_percent'
    fin = open(filename, 'r')
    line = fin.readline()
    protocol_type = {}
    service_type = {}
    ...
    while line != '':
        ...
        if key4 not in labelset.keys():
            labelset[key4] = 0
        labelset[key4] = labelset[key4] + 1
    ...
    print len(labelset.keys()), labelset
    np.savetxt('../../../../dataset/kddcup99-10/kddcup99-10.data.pp.csv', data, delimiter=',', fmt='%.2f')


def load_data():
    fin = open('../../../../dataset/kddcup99-10/kddcup99-10.data.pp.csv', 'r')
    data = np.zeros([494021, 121])
    line = fin.readline()
    cnt = 0
    while line != '':
    ...
    print attack_num, float(attack_num) / 494021
