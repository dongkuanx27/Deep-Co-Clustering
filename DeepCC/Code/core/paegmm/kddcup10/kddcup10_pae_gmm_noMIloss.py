...
class KddcupPaeGmm:
    ...
    def run(self, data, train_epochs):
        ...
        obj_oa = error_oa * 1e1 + train_l2_reg * 1e-2 + loss * 1.5e1 + pen_dev * 1.5e-2
        
        train_step = tf.train.AdamOptimizer(1e-4).minimize(obj_oa)
        #train_step_1 = tf.train.AdamOptimizer(1e-4).minimize(obj_oa)
        ...
        for k in range(epoch_tot):
            train_step.run(feed_dict={keep_prob: 1.0, train_x_v: train_norm_x})
            # elif k < 20000:
            ...
            '''
            fetch = {'obj_oa': obj_oa, 'error_oa':error_oa, 'train_l2_reg':train_l2_reg, 'loss':loss, 'pen_dev':pen_dev, \
                        'train_z':train_z, 'p_z':p_z, \
                        'mixture_dev':mixture_dev, 'mixture_cov':mixture_cov, 'mixture_dev_det':mixture_dev_det, \
                        'x_t':x_t, 'p_t':p_t, 'z_p':z_p, 'z_t':z_t}
            RR = sess.run(fetch,feed_dict={keep_prob: 1.0, train_x_v: train_norm_x})
        ...
        sio.savemat('/storage/work/dux19/Deep_co_clustering/DCC_with_KL_divergence_loss_pretrain/RR_result_noMIloss.mat', {'RR_obj_oa':RR_obj_oa, 'RR_error_oa':RR_error_oa, 'RR_train_l2_reg':RR_train_l2_reg, \
