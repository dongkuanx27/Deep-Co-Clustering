...
class KddcupPaeGmmCompare:
    ...
    def run(self, data, train_epochs):
        ...
        latent = test_z.eval(feed_dict={test_x_v: test_norm_x, keep_prob: 1.0})
        predict_lh = test_likelihood.eval(feed_dict={train_x_v: train_norm_x, test_x_v: test_norm_x, keep_prob: 1.0})
        precision, recall, f1, predict = self.accuracy(predict_lh, num_test_points, test_y, 0.2)
        # print train_out
        ...
        print 'Accuracy: ' + str(precision) + ',' + str(recall) + ',' + str(f1)
